...
class RewardModelTrainer():
    def __init__(self, *, reward_model, policy, query_sampler, hparams, comm):
        ...
        with tf.device(None), tf.device('/cpu:0'):
            with tf.compat.v1.variable_scope('label_buffer', use_resource=True, initializer=tf.compat.v1.zeros_initializer):
                self.train_buffer = utils.SampleBuffer(capacity=hparams.labels.num_train, schemas=data_schemas)

        with tf.compat.v1.name_scope('train_reward'):
            ...
            def train_batch(indices, lr):
                with tf.compat.v1.name_scope('minibatch'):
                    ...
                    with tf.control_dependencies([train_op]):
                        step_var = tf.compat.v1.get_variable(name='train_step', dtype=tf.int64, shape=(), trainable=False, use_resource=True)
...
def train(hparams: HParams):
    with tf.Graph().as_default():
        hyperparams.dump(hparams)  # put these into log
        ...
        hyperparams.dump(m.hparams(), name='model_hparams')  # into log
        ...
        with utils.variables_on_gpu():  # make sure all variables and params are on GPU if any.
            init_ops = tf.group(
