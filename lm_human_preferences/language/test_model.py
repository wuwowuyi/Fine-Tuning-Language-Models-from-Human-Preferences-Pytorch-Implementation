...
def test_incremental():
    ...
    X = tf.compat.v1.placeholder(shape=[batch_size, None], dtype=tf.int32)
    logits = m(X=X)['lm_logits']
    past_p = tf.compat.v1.placeholder(shape=model.past_shape(hparams=hparams, batch_size=batch_size), dtype=tf.float32)
    # Test reusing it in a different variable scope
    with tf.compat.v1.variable_scope('other_scope'):
    ...
    with tf.compat.v1.Session() as sess:
        for nd in 1, 2, 3:
            for ns in range(nd, 4):
                ...
                theirs = tf.linalg.band_part(tf.ones([nd, ns], dtype=tf.int32), tf.cast(-1, tf.int32), ns-nd)
