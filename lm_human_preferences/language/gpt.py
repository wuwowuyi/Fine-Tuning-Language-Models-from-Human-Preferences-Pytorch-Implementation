...
# class GPTConfig:
#     n_ctx: int = 1024
#     n_vocab: int = 50304 # GPT-2 n_vocab of 50257, padded up to nearest multiple of 64 for efficiency
#     n_layer: int = 12
#     n_head: int = 12
#     n_embd: int = 768
#     dropout: float = 0.0
...
class GPT(nn.Module):
    ...
    def crop_block_size(self, block_size):
        ...
        assert block_size <= self.config.n_ctx
        self.config.n_ctx = block_size
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        assert all('pdrop' in k for k in override_args)
        ...
        print("forcing n_vocab=50257, n_ctx=1024, bias=True")
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        sd_keys = [k for k in sd_keys if not (k.endswith('.attn.bias') or k.startswith('hp_head'))] # discard this mask / buffer, not a param
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type, extra_params):
