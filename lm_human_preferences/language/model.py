...
def gelu(x):
    with tf.compat.v1.name_scope('gelu'):
    ...
    with tf.compat.v1.variable_scope(scope):
        *start, nx = utils.shape_list(x)
        ...
        with tf.control_dependencies([x]):
            w = tf.squeeze(tf.compat.v1.get_variable('w', [1, nx, nf], initializer=tf.compat.v1.random_normal_initializer(stddev=w_init_stdev)), axis=0)
...
def attn(x, scope, n_state, *, past, mask, do_dropout, scale=False, hparams, seed):
    ...
    def multihead_attn(q, k, v, *, seed):
        ...
        if scale:
            ...
            w = w * tf.math.rsqrt(tf.cast(n_state, w.dtype))
...
def fc_layer(x, outshape, *, in_axes=1, scale=None):
    ...
    w = tf.compat.v1.get_variable('w', inshape + outshape, initializer=tf.compat.v1.random_normal_initializer(stddev=scale))
    b = tf.compat.v1.get_variable('b', outshape, initializer=tf.compat.v1.constant_initializer(0))
    # Call the regularizer manually so that it works correctly with GradientTape
    regularizer = tf.keras.regularizers.l2(l=0.5 * (1/np.prod(outshape))) #so that initial value of regularizer is 1
...
class Model:
    def __init__(self, hparams: HParams, scalar_heads=[], scope=None):
        ...
        with tf.compat.v1.variable_scope(scope, 'model') as scope:
            self.scope = scope
        self.built = False
    ...
    def __call__(self, *, X, Y=None, past=None, past_tokens=None, mask=None,
        ...
        if padding_token is not None:
            ...
            X = tf.compat.v1.where(mask, X, tf.zeros_like(X))
            if past is not None:
        ...
        with tf.compat.v1.variable_scope(self.scope, reuse=self.built, auxiliary_name_scope=not self.built):
            self.built = True
            results = {}
            ...
            wpe = tf.compat.v1.get_variable('wpe', [self.hparams.n_ctx, self.hparams.n_embd],
                                  initializer=tf.compat.v1.random_normal_initializer(stddev=0.01))
            wte = tf.compat.v1.get_variable('wte', [self.hparams.n_vocab, self.hparams.n_embd],
            ...
            if mask is not None:
                ...
                with tf.control_dependencies([tf.compat.v1.assert_none_equal(use_indices, -1)]):
            ...
            for head_name, head_seed in zip(self.scalar_heads, head_seeds):
                with tf.compat.v1.variable_scope(f"heads/{head_name}"):
                    dropped_h = \
